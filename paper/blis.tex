...
$\tilde p^{\mathrm{pf},(m)}(t)$ constructed from the 
current iterate $\beta^{(m)}$ (Section~\ref{sec:beta_informed_prefill_correction}).
...
At iteration $m$, we treat the current coefficient vector $\beta^{(m)}$
...
$\bar p_i^{(\cdot),(m)}$) are then held fixed while solving for $\beta^{(m+1)}$
via an NNLS subproblem.

Given instantaneous pressures, define the iteration-$m$ local step-time model
\begin{equation}
\Delta^{(m)}(t)
=
\beta^{(m)}_0
+
\beta^{(m)}_1\,\tilde p^{\mathrm{pf},(m)}(t)
+
\beta^{(m)}_2\,p^{\mathrm{dec}}(t),
\qquad \Delta^{(m)}(t) > 0.
...
This iteration-indexing eliminates circularity: $\lambda^{(m)}$ is computed
from $\beta^{(m)}$ and iteration-$m$ effective pressures, and is not defined
implicitly in terms of itself.
...
(prefill) partial chunk as exogenous. In contrast, our iterative estimator
uses the current iterate $\beta^{(m)}$ to construct a trace-only step-density
proxy $\lambda^{(m)}(t)$ and uses it to place the missing prefill token mass
$\mu_r$ near the \emph{latent final prefill step}.

\paragraph{Step-time attribution density.}
Using $\lambda^{(m)}(t)$ from Eq.~\eqref{eq:step_density_m}, define the normalized
density over request $r$'s prefill window:
\begin{equation}
q_r^{(m)}(t)
=
...
\label{eq:qr_def_m}
\end{equation}
We interpret $q_r^{(m)}$ as a \emph{trace-only step-time attribution model}
within the prefill window, computed from $\beta^{(m)}$ and held fixed during
the NNLS update for $\beta^{(m+1)}$.

\paragraph{Last-step window under $q_r^{(m)}$.}
...
chunk (Eq.~\eqref{eq:rho_def}). Define the $q_r^{(m)}$-CDF
\begin{equation}
F_r^{(m)}(t)=\int_{t_{r,s}}^{t} q_r^{(m)}(u)\,du,
\label{eq:Fr_def_m}
\end{equation}
and define the \emph{last-step window} as the final $1/N_r$ fraction of step exposure:
\begin{equation}
\mathcal{W}_r^{(m)}
=
\Bigl\{
...
\paragraph{Iteration-$m$ correction signal and induced effective pressure.}
We concentrate the missing token mass $\mu_r$ on $\mathcal{W}_r^{(m)}$
proportional to $q_r^{(m)}(t)$, expressed as a \emph{correction signal}
(tokens/second) whose time integral equals $\mu_r$.

Define the per-request correction signal
\begin{equation}
c_r^{(m)}(t)
=
\mu_r\,
...
\end{equation}
and the aggregate correction $c^{(m)}(t)=\sum_r c_r^{(m)}(t)$.
By construction, $\int c_r^{(m)}(t)\,dt=\mu_r$.

To use this correction inside the local step-time model (Eq.~\eqref{eq:local_step_time_m}),
convert the time-based correction signal into a tokens/step decrement by dividing by
the iteration-$m$ step density $\lambda^{(m)}(t)$:
\begin{equation}
...
All quantities above enter only through integrals over the trace-induced grid
...
We treat $q_i^{(m)}$ as a trace-only step-time attribution model within the
phase window, computed from $\beta^{(m)}$ and held fixed during the NNLS update.
...
\label{eq:sr_nnls_m}
\end{equation}
where $\bar p^{\mathrm{pf},(m)}_i$ and $\bar p^{\mathrm{dec},(m)}_i$ are computed from
$\beta^{(m)}$ via Eqs.~\eqref{eq:local_step_time_m}--\eqref{eq:step_averaged_pressures_m}
...
With this modification, each iteration still forms trace-derived features using
$\beta^{(m)}$ and then solves a convex subproblem with those features held fixed,
yielding an update of the form $\beta^{(m+1)}=\mathcal{F}(\beta^{(m)})$.
We terminate when $\|\beta^{(m+1)}-\beta^{(m)}\|_2<\epsilon$ (or relative change)
