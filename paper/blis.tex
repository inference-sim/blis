...
$N_i = \lceil P_i / C \rceil$,
where $P_i$ denotes the number of prefill tokens that induce computation.\footnote{
Prefix-cached tokens do not contribute to execution steps and are therefore
excluded from $P_i$. This treatment is consistent with the execution semantics
...
Although we define token pressures $p^{\mathrm{pf}}(t)$, $p^{\mathrm{dec}}(t)$ and
the partial-chunk correction weights $w_r(t)$ as functions of wall-clock time,
the baseline estimator never requires evaluating them instant by instant.
It uses these objects only through time integrals over phase intervals, e.g.,
$\int_{t_{i,s}}^{t_{i,e}} \tilde p^{\mathrm{pf}}(t)\,dt$ and
$\int_{t_{i,s}}^{t_{i,e}} p^{\mathrm{dec}}(t)\,dt$.
In practice, these integrals can be computed exactly from the trace by sweeping
over the sorted phase-boundary events and accumulating piecewise-constant
overlap counts (and, for prefill, applying a per-request partial-chunk
adjustment whose placement is irrelevant in the baseline as long as its total
mass is preserved).
