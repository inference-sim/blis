...
\begin{itemize}
  ...
  \item a trace-inferred step count $N_i \in \mathbb{N}$, deterministically
        derived from trace-visible token counts together with known engine
        scheduling rules.
\end{itemize}

The step count $N_i$ is not directly observed but is inferred as follows.
...
a standard trace-consistent choice is
$N_i = \lceil P_i / C \rceil$, where $P_i$ denotes the number of prefill tokens
that induce computation.\footnote{Prefix-cached tokens do not contribute to
execution steps and are therefore excluded from $P_i$.}

The estimation problem is therefore to infer the step-level execution
coefficients $\beta$ in Eq.~\eqref{eq:step_model} using only phase-level
summaries—namely, observed phase durations, aggregate token counts, and
trace-inferred step counts—without access to per-step timings or internal
