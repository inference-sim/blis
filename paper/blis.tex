...
\section{Unified Allocation--Refit Estimation}
\label{sec:allocation-refit}

This section introduces a unified estimation framework for calibrating
request-level latency and throughput models from partial, overlapping
observations of inference execution.
While the literature admits many estimation procedures under such
incomplete observability, we show that the seemingly disparate estimators
used in practice can all be viewed as instances of a single
\emph{allocation--refit} principle.

At a high level, inference traces provide aggregate measurements over
unknown, dynamically interleaved execution intervals (e.g., prefill and
decode phases across concurrent requests).
The core challenge is therefore not parameter fitting per se, but
\emph{allocating observed aggregate execution time to latent execution
quanta} before fitting parametric models.

We formalize this intuition as a two-stage iterative procedure:
\begin{enumerate}
    \item \textbf{Allocation:} infer the fractional contribution of each
    latent execution unit (e.g., per-step or per-token work) to observed
    aggregate measurements;
    \item \textbf{Refit:} update parametric coefficients given the inferred
    allocation.
\end{enumerate}

This allocation--refit structure induces a natural
\emph{generalized expectation--maximization (GEM)} interpretation:
the allocation step corresponds to computing soft responsibilities over
latent execution structure, while the refit step updates model parameters
given these responsibilities.
Different estimators arise solely from different choices of
allocation kernels, refit solvers, and robustness constraints.

Importantly, this perspective reveals that many estimators commonly
treated as distinct---including non-negative least squares baselines,
step-density reweighting methods, kernelized EM variants, and robust
iteratively reweighted solvers---are not competing algorithms, but
\emph{approximations or specializations of the same underlying GEM
procedure}.

In the remainder of this section, we proceed as follows.
We first introduce a simple baseline estimator that serves as a
principled initialization and sanity check (\S\ref{subsec:baseline}).
We then formalize the allocation--refit principle and its GEM
interpretation (\S\ref{subsec:gem}).
Finally, we present a continuous kernelized allocation--refit algorithm
that we treat as the canonical BLIS estimator (\S\ref{subsec:continuous}),
with robust and approximate variants discussed as extensions.

\subsection{Allocation--Refit as a Generalized EM Procedure}
\label{subsec:gem}

We now formalize the \emph{allocation--refit} principle introduced in
\S\ref{sec:allocation-refit}.
This procedure constitutes the core estimation algorithm in BLIS.
All estimators considered in this paper---including the baseline
initialization, kernelized variants, and robust extensions---are instances
or approximations of this generalized expectation--maximization (GEM)
framework.

\paragraph{Latent execution structure.}
We consider inference execution as composed of latent execution units
indexed by $i$ (e.g., per-step or per-token work), whose contributions to
observed aggregate measurements are not directly observable.
For each request $r$, let $T_r$ denote the observed aggregate execution
time, which arises from a superposition of these latent contributions
under dynamic overlap and concurrency.

We introduce latent allocation variables
$\{z_{ri}\}_{i}$, where $z_{ri} \in [0,1]$ denotes the fractional
contribution of execution unit $i$ to the observed aggregate $T_r$.
These variables satisfy the conservation constraint
\begin{equation}
\sum_i z_{ri} = 1
\qquad \text{for all } r,
\label{eq:allocation_conservation}
\end{equation}
ensuring that each observed aggregate measurement is fully allocated
across latent execution units.

\paragraph{Allocation step (E-step).}
Given current parameter estimates $\beta^{(k)}$, the allocation step
computes soft responsibilities
\begin{equation}
z_{ri}^{(k)}
\;\propto\;
\mathcal{K}_r(i;\beta^{(k)}),
\label{eq:allocation_step}
\end{equation}
where $\mathcal{K}_r(\cdot)$ is an allocation kernel that encodes prior
knowledge about execution overlap, temporal locality, and interference.
The kernel is normalized to satisfy
\eqref{eq:allocation_conservation}.
Different choices of $\mathcal{K}_r$ correspond to different instantiations
of the same allocation--refit principle.

\paragraph{Refit step (M-step).}
Given allocation weights $\{z_{ri}^{(k)}\}$, the refit step updates the
model parameters by solving
\begin{equation}
\beta^{(k+1)}
=
\arg\min_{\beta \ge 0}
...
T_r
-
\sum_i z_{ri}^{(k)}\,\beta_i
\right)^2
+
\mathcal{R}(\beta),
\label{eq:refit_step}
\end{equation}
where $\mathcal{R}(\beta)$ denotes optional regularization or robustness
terms.
This step corresponds to a weighted least-squares update and is convex
under standard choices of $\mathcal{R}$.

\paragraph{Generalized EM interpretation.}
Together, the allocation and refit steps define a generalized EM
procedure.
The allocation step computes soft responsibilities over latent execution
structure, while the refit step updates parameters given these
responsibilities.
Because the refit step need only decrease (rather than globally minimize)
the objective, this formulation naturally accommodates approximate solvers,
robust losses, and constrained updates.

The baseline estimator introduced in \S\ref{subsec:baseline} corresponds
to a degenerate single-pass instance of this procedure, in which allocation
weights are fixed a priori and no iterative refinement is performed.
Conversely, more expressive allocation kernels yield progressively more
accurate attribution under concurrent execution.

\paragraph{Canonical instantiation.}
Among the many possible instantiations of this meta-algorithm, we treat
the continuous kernelized allocation--refit procedure introduced in
\S\ref{subsec:continuous} as the canonical BLIS estimator.
This formulation preserves exact mass conservation while avoiding
discretization artifacts, and serves as the primary focus of our empirical
evaluation.
...
\subsection{Baseline Allocation for Initialization}
\label{subsec:baseline}

We begin with a simple baseline estimator that serves as a
\emph{principled initialization} for the allocation--refit procedure
introduced in \S\ref{subsec:gem}.
This estimator is not intended as a final solution, but rather as a
single-pass approximation that establishes feasibility, identifiability,
and a stable starting point for subsequent refinement.

\paragraph{Problem setting.}
Consider a collection of inference requests indexed by $r$.
Each request is associated with an observed aggregate execution time
$T_r$, corresponding to the total time spent in a given execution phase
(e.g., prefill or decode).
This aggregate time is the sum of contributions from a sequence of latent
execution units indexed by $i$ (e.g., per-step or per-token work),
with unknown per-unit coefficients $\beta_i \ge 0$.

Let $N_{ri}$ denote the known count of execution units of type $i$
contributing to request $r$.\footnote{In practice, $N_{ri}$ may correspond
to token counts, step counts, or other discrete workload measures derived
from traces or model metadata.}
The aggregate observation can then be written as
\begin{equation}
T_r \;=\; \sum_i N_{ri}\,\beta_i + \varepsilon_r,
\label{eq:baseline_model}
\end{equation}
where $\varepsilon_r$ captures measurement noise and unmodeled effects.

\paragraph{Baseline estimator.}
The baseline estimate $\widehat{\beta}^{(0)}$ is obtained by solving the
...
\sum_r
\left(
T_r - \sum_i N_{ri}\,\beta_i
\right)^2.
\label{eq:baseline_nnls}
\end{equation}
This optimization is convex and admits efficient solvers.
The non-negativity constraint enforces physical plausibility and ensures
that the resulting coefficients correspond to valid execution costs.

\paragraph{Interpretation as degenerate allocation--refit.}
From the perspective of the allocation--refit framework, the baseline
estimator corresponds to a \emph{degenerate single-pass instance} in which
allocation weights are fixed a priori.
Specifically, each execution unit $i$ contributing to request $r$ is
assigned a uniform fractional responsibility proportional to $N_{ri}$,
and no iterative reallocation is performed.
The refit step then reduces to solving \eqref{eq:baseline_nnls} once.

This interpretation clarifies the role of the baseline: it performs a
coarse attribution of aggregate execution time to latent units under the
assumption of uniform overlap and no temporal interference.
While this assumption is violated under concurrent execution, the resulting
coefficients provide a stable and interpretable initialization.

\paragraph{Role in the overall estimation pipeline.}
In the remainder of this paper, we use $\widehat{\beta}^{(0)}$ exclusively
as an initialization for iterative allocation--refit procedures.
All subsequent estimators refine these coefficients by explicitly modeling
overlap, interference, and partial observability through soft allocation
kernels.
Empirically, we find that this initialization accelerates convergence and
improves numerical stability, while having negligible impact on final
estimates once refinement is applied.

\subsection{Continuous Kernelized Allocation--Refit}
\label{subsec:continuous}

We now present the continuous kernelized allocation--refit procedure,
which we treat as the \emph{canonical BLIS estimator}.
This formulation instantiates the generalized allocation--refit framework
(\S\ref{subsec:gem}) using continuous allocation kernels that preserve
exact mass conservation while avoiding discretization artifacts inherent
to grid-based approaches.

\paragraph{Continuous latent execution timeline.}
We model inference execution on a continuous latent timeline
$t \in \mathbb{R}_{\ge 0}$.
Each request $r$ occupies an unknown execution interval
$[s_r, e_r] \subset \mathbb{R}_{\ge 0}$, corresponding to a given execution
phase (e.g., prefill or decode).
Observed aggregate execution times $T_r$ correspond to integrals over
latent execution cost densities along this timeline.

Let $\beta(t) \ge 0$ denote the instantaneous execution cost density.
The aggregate observation can be written as
\begin{equation}
T_r
=
\int_{s_r}^{e_r}
...
\end{equation}
where $\varepsilon_r$ captures measurement noise.

\paragraph{Kernelized allocation.}
Given a current estimate $\beta^{(k)}(t)$, we define a normalized
allocation kernel
\begin{equation}
z_r^{(k)}(t)
=
...
\qquad
t \in [s_r, e_r],
\label{eq:continuous_allocation}
\end{equation}
where $\phi_r(t) \ge 0$ is a request-specific kernel encoding prior
knowledge about temporal locality, boundary uncertainty, or execution
structure.
By construction,
\begin{equation}
\int_{s_r}^{e_r} z_r^{(k)}(t)\,dt = 1,
\label{eq:continuous_conservation}
\end{equation}
ensuring exact mass conservation.

\paragraph{Refit via weighted regression.}
Given allocation densities $\{z_r^{(k)}(t)\}$, we update the execution cost
density by minimizing the functional objective
\begin{equation}
\beta^{(k+1)}
=
\arg\min_{\beta \ge 0}
...
\int
\|\nabla \beta(t)\|^2\,dt.
\label{eq:continuous_refit}
\end{equation}
The regularization term promotes smoothness and numerical stability, and
may be omitted or replaced depending on modeling assumptions.
In practice, $\beta(t)$ is represented in a finite-dimensional basis
(e.g., piecewise constant or spline basis), reducing
\eqref{eq:continuous_refit} to a convex optimization problem.

\paragraph{Relationship to discrete and deterministic variants.}
Discrete grid-based EM procedures arise as special cases of this
formulation when $\beta(t)$ is constrained to a fixed micro-grid and
$\phi_r(t)$ is chosen to be piecewise constant.
Similarly, deterministic step-density reweighting and MM-style updates
correspond to replacing the probabilistic allocation
\eqref{eq:continuous_allocation} with hard or heuristic assignments.
The continuous kernelized formulation strictly generalizes these
approaches while avoiding grid-alignment artifacts and sensitivity to
discretization resolution.

\paragraph{Convergence and stability.}
Each iteration of the continuous allocation--refit procedure decreases
the aggregate reconstruction error in \eqref{eq:continuous_refit},
subject to regularization.
Empirically, we observe rapid convergence (typically within a small
number of iterations) when initialized from the baseline estimator
(\S\ref{subsec:baseline}).
The continuous formulation is particularly stable under heavy overlap
and partial observability, as allocation mass is redistributed smoothly
rather than discretely.

\paragraph{Summary.}
The continuous kernelized allocation--refit procedure provides a
principled, numerically stable, and implementation-friendly instantiation
of the BLIS framework.
It serves as the primary estimator evaluated in this paper, with other
variants interpreted as approximations or extensions under specific
practical constraints.

\subsection{Variants and Extensions}
\label{subsec:variants}

The continuous kernelized allocation--refit procedure presented in
\S\ref{subsec:continuous} serves as the canonical BLIS estimator.
In practice, however, operational constraints such as robustness
requirements, observability limitations, or deployment considerations
may motivate alternative instantiations.
We now briefly describe several such variants, all of which are naturally
interpreted as approximations or extensions of the same underlying
allocation--refit framework.

\paragraph{Robust refit via iteratively reweighted least squares.}
In environments with heavy-tailed noise or transient interference,
the squared-error objective in the refit step
(\eqref{eq:continuous_refit}) may be sensitive to outliers.
A robust alternative replaces the quadratic loss with a convex
robust loss (e.g., Huber or Tukey), yielding an iteratively reweighted
least-squares (IRLS) update.
Within the allocation--refit framework, this modification affects only
the refit step and preserves the generalized EM structure.

\paragraph{Discrete and grid-based approximations.}
Discrete grid-based EM procedures arise when the latent execution timeline
is discretized into fixed micro-intervals and $\beta(t)$ is constrained
to be piecewise constant on this grid.
While such discretizations simplify implementation, they introduce
resolution-dependent artifacts and boundary sensitivity.
These methods are best understood as numerical approximations to the
continuous kernelized formulation, and are therefore omitted from the
main presentation.

\paragraph{Deterministic and MM-style updates.}
Deterministic step-density reweighting and minorization--maximization
(MM) procedures correspond to replacing the probabilistic allocation
kernel with hard or heuristic assignments.
Although these updates may be computationally convenient, they sacrifice
the soft attribution and uncertainty propagation provided by the GEM
formulation.
We view such methods as coarse approximations suitable primarily for
exploratory analysis or constrained environments.

\paragraph{Partial observability and client-side measurements.}
In some deployments, fine-grained execution traces are unavailable and
only client-observed end-to-end latencies can be collected.
The allocation--refit framework naturally accommodates this setting by
treating unobserved execution intervals as latent and allocating client
measurements over feasible execution windows.
This variant preserves the same algorithmic structure while operating
under weaker observability assumptions.

\paragraph{Featureized and hierarchical extensions.}
For transfer across models, hardware platforms, or software versions,
the refit step may be augmented with featureized or hierarchical
parameterizations.
In this setting, $\beta$ is modeled as a function of system- or
request-level features, and the refit step corresponds to updating shared
latent representations rather than environment-specific coefficients.
This extension enables cold-start calibration and multi-environment
generalization without altering the allocation--refit backbone.

\paragraph{Summary.}
All variants discussed above preserve the core allocation--refit
structure and differ only in the choice of allocation kernel,
refit objective, or parameterization.
Accordingly, we treat them as extensions of the canonical BLIS estimator
rather than independent algorithms.
Our empirical evaluation focuses on the continuous kernelized formulation,
with selected variants examined to illustrate robustness and deployment
tradeoffs.
