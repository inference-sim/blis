...
We estimate the model coefficients through a set of scenarios as follows. Each scenario defines a 
set of conditions that the inference workload needs to satisfy.

\subsubsection{Scenario 1}
\label{subsubsec:scenario1}

\begin{enumerate}
    \item Requests arrive strictly sequentially: request $i+1$ is only emitted after
    the final response for request $i$ has been received.
    \item Each request consists of a single prompt (prefill only).
...
Under these conditions, the end-to-end latency model reduces to a simplified form:
