...
The missing mass $\Delta_r$ should be subtracted from the prefill pressure
...
Let $t_{r,\mathrm{pf\mbox{-}end}}$ denote the trace-visible end time of request
$r$’s prefill phase.
Define a nonnegative correction weight $w_r(t)$ supported near
$t_{r,\mathrm{pf\mbox{-}end}}$ and normalized such that
\[
\int w_r(t)\,dt = 1.
\]
...
This estimator fits wall-clock--weighted behavior.
Periods with long steps exert disproportionate influence, and the estimator
converges to time-averaged rather than step-averaged execution costs.
...
The trace-inferred step count $N_i$ is treated as fixed; $q_i$ estimates where
those steps occur in time.
...
$\Delta_r$ & Missing prefill chunk mass & tokens \\
