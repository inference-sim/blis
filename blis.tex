...
\begin{figure}[!ht]
  \centering
  \includegraphics[height=0.55\textheight]{figs/lifecycle.pdf}
  \caption{Request Lifecycle}
  \label{fig:req-lifecycle}
\end{figure}

The life of an inference request in the system is illustrated in 
Figure \ref{fig:req-lifecycle} and described below.
...
sufficient GPU capacity. Let $\mathcal{S}_i$ be the set of busy loop iterations 
during this period. Let $\lambda_j$ denote the duration of busy loop iteration $j$. 
The duration of this waiting period equals $\sum_{j \in \mathcal{S}_i} \lambda_j$.

\paragraph{4. Chunked prefill.} Let $\mathcal{P}_i$ be the set of  
busy loop iterations in which the request participated in its prefill phase.
The prefill duration equals $\sum_{j \in \mathcal{P}_i} \lambda_j$. If the 
input sequence of the request contains $x$ uncached tokens, and the chunk size is $C$, 
then $\lvert \mathcal{P}_i \rvert = \lceil x/C \rceil$.

\paragraph{5. Decode.} Let $\mathcal{D}_i$ be the set of 
busy loop iterations in which in which the request participated in its decode phase.
The decode duration equals $\sum_{j \in \mathcal{D}_i} \lambda_j$. The first output
token is generated during the prefill phase, and subsequent tokens are generated 
in the decode iterations. Thus, $\lvert \mathcal{D}_i \rvert = t_i^{out} - 1$.

\paragraph{6. Post-process.} Once all output tokens are produced, 
the API server detokenizes them and
packages the final response. This latency depends linearly on the 
...
Let $\chi_{j,i}^{\text{hit}}$ and $\chi_{j,i}^{\text{miss}}$ denote the number of 
cached and uncached tokens (hits and misses) respectively 
for request $i$ during iteration $j$. Define $\psi_j$ as follows.

\begin{align}
\psi_j
  = \sum_{i | \text{j is prefill iteration for i}}
    \Big(
...
We model the duration of iteration $j$ 
as a quadratic function of queue, batch, 
and cache related features as follows:
\begin{align}
\lambda_j & \approx \beta_0 \nonumber \\
 & + \beta_1 \times \sum_{i | \text{j is first prefill iteration for i}} \chi_{j,i}^{\text{hit}} \nonumber \\
 & + \beta_2 \times \sum_{i | \text{j is prefill iteration for i}} \chi_{j,i}^{\text{miss}} \nonumber \\
 & + \beta_3 \times \psi_j \nonumber \\
 & + \beta_4 \times \text{(number of decode requests in j)} \nonumber \\ 
 & + \beta_5 \times \text{(total length of decode requests in j)} \nonumber \\
 & + \beta_6 \times \text{(number of requests finishing in j)} \nonumber \\ 
 & + \beta_7 \times \text{(total length of requests finishing in j)} 
\label{eq:busy-loop-expanded}
\end{align}

\noindent\textbf{Justification:} Each busy-loop step mixes four kinds of work.
...
the tokens to determine the number of cache hits (linear in hits; \(\beta_1\)).
...
\(\beta_3\)).
  \item \emph{Decode work.} Decode emits at most one token per active request:
...
a block size $b \in \{8, 16, 32, 64, 128\}$, and maximum sequence length 
$M = 8192 / b$ blocks.  

Each quad $i$ consists of four requests $(i,1)$, $(i,2)$, $(i,3)$, and $(i,4)$, 
each comprising one or more segments. Requests within a quad may share prefixes, 
while requests across quads do not. 
...
\begin{enumerate}
  \item $(i,1)$ is a prefill-only request with a single input segment of length 
        $1 + \Delta_{i,1}$ blocks. Its first four tokens are distinct from the first four
        tokens of any request in quads $\{1, 2, \ldots, i-1 \}$. This guarantees that
        request $(i, 1)$ does not share a prefix with any previously generated requests. Its
        maximum output length is set to $1$ token.
  \item $(i,2)$ is a prefill-only request with two input segments, of total length 
        $2 + \Delta_{i,1} + \Delta_{i,2}$ blocks. 
        The first segment is identical to $(i,1)$; the second has length 
        $1 + \Delta_{i,2}$ blocks. Its
        maximum output length is set to $1$ token.
  \item $(i,3)$ reuses the input of $(i,1)$. Its maximum output length is set to $1 + \Delta_{i,3}$ blocks.
  \item $(i,4)$ has two input segments. The first segment is identical to $(i,1)$.  
        The second segment has length $1 + \Delta_{i,2}$ blocks, but its first token  
        differs from the first token of the second segment in $(i,2)$. This guarantees that request 
        $(i, 4)$ shares a prefix of length exactly $1 + \Delta_{i,1}$ blocks 
        with other requests in this quad.
        Its maximum output length is set to $1 + \Delta_{i,3}$ blocks.
\end{enumerate}
...
\begin{table}[h]
\centering
\caption{Structure of requests in quad in Scenario~3. 
Unless specified otherwise, lengths are in blocks; ``='' indicates a segment 
...
$(i,1)$ & $[\,1+\Delta_{i,1}\,]$ 
        & $1+\Delta_{i,1}$ 
        & 1 token (prefill only) \\
$(i,2)$ & $[\,= (i,1)\,],\,[\,1+\Delta_{i,2}\,]$ 
        & $2+\Delta_{i,1}+\Delta_{i,2}$ 
        & 1 token (prefill only) \\
$(i,3)$ & $[\,= (i,1)\,]$ 
        & $1+\Delta_{i,1}$ 
        & $1+\Delta_{i,3}$ \\
...
\paragraph{Sampling.}
Requests in a quad have a fixed length budget of $M$ blocks that must be divided across input segments and output, 
...
\begin{algorithm}[th]
  ...
  \begin{algorithmic}[1]
    \Require Budget in blocks $M-3$; concentration $\boldsymbol{\eta}=(\eta_1,\eta_2,\eta_3)$
