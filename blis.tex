...
communicate through a scheduling queue.
...
\begin{enumerate}
    ...
    \item Enqueuing tokenized requests into the scheduling queue.
    ...
    \item Detokenizing completed sequences and packaging them into response
          objects.
    \item Emitting the response back to the client.
\end{enumerate}
...
validation, and enqueues the request into the scheduling queue contributing a latency of $m_i$. This has a linear dependence 
...
\paragraph{5. Decode.} The duration of prefill equals the
...
$m_i$ & Enqueue latency (API $\to$ message queue) for request $i$ \\
...
$c_i$ & Completion latency (post-processing / packaging) for request $i$ \\
...
$\chi_{j,i}^{\text{hit,new}}$ & Cache hits (blocks) in \emph{new} prefill blocks for request $i$ at $j$ \\
$\chi_{j,i}^{\text{miss}}$ & Cache misses (blocks) in \emph{new} prefill blocks for request $i$ at $j$ \\
$\chi_{j,i}^{\text{hit,all}}$ & \emph{Total} cache hits (all blocks) for request $i$ at $j$ \\
...
\begin{align}
e_i &= n_i^{\text{in}} + m_i + s_i + p_i + d_i + c_i + n_i^{\text{out}}
\label{eq:end-to-end}
\end{align}

\paragraph{Parametric regression form.}
\begin{align}
e_i
&= \alpha_0
 + \alpha_1\, t_i^{\text{in}}
