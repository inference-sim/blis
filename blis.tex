...
packages the final response. This incurs a completion latency $c_i$, with a linear dependence on 
...
$\chi_{j,i}^{\text{hit,new}}$ & Cache hits (blocks) in \emph{new} prefill blocks for request $i$ at $j$; this is an integer \\
$\chi_{j,i}^{\text{miss}}$ & Cache misses (blocks) in \emph{new} prefill blocks for request $i$ at $j$; \\
                           & for ease of estimation, we define this not as an integer, but as the ratio between \\
                           & number of tokens involved in the cache miss and the block size \\
$\chi_{j,i}^{\text{hit,all}}$ & \emph{Total} cache hits (all blocks) for request $i$ at $j$; this is an integer \\
...
We first define the quadratic cache penalty term that captures nonlinear
interactions between cache \emph{misses} in newly admitted prefill blocks and
\emph{total} cache hits during iteration $j$:
\begin{align}
\psi_j
  = \sum_{i \in \mathcal{B}_j}
    \Big(
      \chi_{j,i}^{\text{miss}} \cdot \chi_{j,i}^{\text{hit,all}}
      + \big(\chi_{j,i}^{\text{miss}}\big)^2
    \Big)
\label{eq:quad}
\end{align}

The duration of iteration $j$ is now modeled as a quadratic function of features derived from queue, batch, 
and cache shape features:
\begin{align}
...
We estimate the model coefficients through a set of scenarios as follows. Each scenario involves a 
sequence of inference requests emitted by the client such that a few conditions hold.

\subsubsection{Scenario 1}
\label{subsubsec:scenario1}

\begin{enumerate}
      \item $\forall i$, client emits request $i+1$ only after final repsonse for request $i$ is received.
      \item Each request is a prefill request, and has a single prompt.
      \item Each prompt has a length that is $\leq$ chunk size.
      \item No two prompts (across requests in the workload) have a prefix match. 
      This condition holds if and only if the sequence of tokens in the first block of each request is unique.
\end{enumerate}

The above scenario leads to a simplified system of equations as follows.

\begin{align}
 \forall i \in \text{Scenario 1}:\quad 
   e_i &= (\alpha_0 + \beta_0 + \beta_1 + \beta_3 + \beta_9 + \beta_{11}) + 
   (\alpha_1 + \beta_2 + \frac{\beta_5}{b} + \beta_{11}) \cdot t_i^{\text{in}} + \beta_6 \left( \frac{t_i^{\text{in}}}{b} \right)^2 \nonumber \\
   &= \gamma_0 + \gamma_1 t_i^{\text{in}} + \beta_6 \left( \frac{t_i^{\text{in}}}{b} \right)^2 \label{eqn:scenario1} \\
 \text{where}\quad 
