...
Second, we refine this estimator using an iterative, step-resolved
reweighting procedure inspired by majorization--minimization (MM).
Finally, we provide intuitive mathematical justification, discuss robustness,
and outline practical limitations and calibration considerations.
...
Steps are assumed to execute sequentially, with instantaneous request
overlaps summarized by aggregate token loads.
...
simulator.
Accurate simulation therefore requires identifying the coefficient vector
...
% ============================================================
\subsection{Trace-Derived Token Pressures and Prefill Correction}
\label{sec:pressures}

To relate phase-level observations to step-level execution, we reconstruct
\emph{token pressures} from trace-visible overlaps among requests.
...
\begin{itemize}
  ...
  \item $p^{\mathrm{dec}}(t)$ denote the total number of decode tokens
        processed \emph{per step} at time $t$.
\end{itemize}
These functions are deterministically reconstructed from traces using
...
\paragraph{Prefill chunking correction.}
...
\tilde C_r(k) =
\begin{cases}
C, & \text{non-final prefill steps},\\
\rho_r, & \text{final prefill step}.
\end{cases}
\end{equation}

Because traces do not reveal which busy-loop iteration is the final prefill
step of a request, we approximate its effect by uniformly redistributing the
fractional mass ratio $\rho_r/C$ across the prefill segment of the request.
This preserves total prefill token mass while avoiding boundary bias, at the
cost of smoothing localized end-of-prefill effects.

Let $\tilde p^{\mathrm{pf}}(t)$ denote the resulting effective prefill pressure.
...
This estimator is intentionally \emph{not} step-resolved and serves as a
reference point for more detailed methods.

\paragraph{Integrated pressures.}
...
Even with unlimited data, this estimator converges to coefficients that are
weighted by wall-clock time rather than by step count.
As a result, periods in which steps are long exert disproportionate influence,
and the estimator matches time-integrated behavior rather than true
step-averaged execution costs.

% ============================================================
\subsection{Step-Density Reweighted Estimation}
\label{sec:step_density}

We now introduce an iterative estimator that moves from wall-clock--weighted
fitting toward step-resolved fitting by explicitly modeling how step density
varies over time.
...
\end{equation}
In practice we enforce $\Delta(t;\beta) \ge \varepsilon > 0$ for numerical
stability.
The corresponding step density (steps per unit time) is
\begin{equation}
...
For phase instance $i$, define the total step mass
\begin{equation}
\Lambda_i(\beta)
=
\int_{t_{i,s}}^{t_{i,e}} \lambda(t;\beta)\,dt.
\end{equation}
We then define a normalized density
\begin{equation}
q_i(t;\beta)
=
\frac{\lambda(t;\beta)}{\Lambda_i(\beta)},
\qquad
\int_{t_{i,s}}^{t_{i,e}} q_i(t;\beta)\,dt = 1.
\label{eq:qi_density}
\end{equation}

\paragraph{Interpretation.}
We emphasize that $N_i$ is a trace-inferred step count and is treated as fixed
throughout.
The quantity $\Lambda_i(\beta)$ is used only to normalize $\lambda(t;\beta)$
into a within-phase weighting $q_i(t;\beta)$.
Intuitively, $q_i$ estimates \emph{where in time} the $N_i$ steps occur, not
\emph{how many} steps occurred.

\subsubsection{Step-Averaged Pressures}

Using $q_i(t;\beta)$, we define step-averaged pressures:
\begin{equation}
...
\end{equation}
These quantities represent expected per-step token loads.
...
\label{eq:sr_nnls}
\end{equation}
For numerical stability we optionally apply damping
$\beta^{(r+1)} \leftarrow (1-\eta)\beta^{(r)} + \eta\beta^{(r+1)}$.

We initialize $\beta^{(0)}$ using the baseline NNLS estimator
(Section~\ref{sec:baseline}).

% ============================================================
\subsection{Robust Step-Density Reweighted Estimation}
\label{sec:robust_step_density}

To mitigate rare but unavoidable outliers in production traces (e.g.,
transient host interference or delayed scheduling), we replace the squared
loss with a robust alternative.

Let the iteration-conditioned residual be
\begin{equation}
r_i^{(r)}(\beta)
=
N_i\beta_0
...
We use the Huber loss $\rho_\delta(\cdot)$ and solve
\begin{equation}
\beta^{(r+1)}
=
\arg\min_{\beta\ge0}
\sum_{i\in\mathcal{I}}
\rho_\delta\!\left(r_i^{(r)}(\beta)\right).
\end{equation}
This problem is convex and can be solved efficiently using standard methods.

% ============================================================
\subsection{Fixed-Point Viewpoint and Stopping Criterion}
...
N_i\beta'_0
+
N_i\bar p^{\mathrm{pf}}_i(\beta)\beta'_1
+
N_i\bar p^{\mathrm{dec}}_i(\beta)\beta'_2
-
T_i
\right).
\end{equation}
The algorithm iterates $\beta^{(r+1)}=\mathcal{F}(\beta^{(r)})$.

We terminate when
\[
\frac{\|\beta^{(r+1)}-\beta^{(r)}\|_2}{\|\beta^{(r)}\|_2} \le \tau
\]
or after a fixed iteration budget.

% ============================================================
\subsection{Limitations and Scope}
\label{sec:limitations}

This approach assumes a single sequential busy-loop and relies on accurate
knowledge of engine scheduling rules.
Uniform redistribution of the final prefill chunk smooths localized effects,
and identifiability requires sufficient variation in token pressures.
...
model (Eq.~\ref{eq:step_model}) of the discrete-event simulator described in
Section~\ref{sec:simulator_design}.
...
require knowledge of the true execution coefficients.
