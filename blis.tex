...
modeling perspective, this has a linear dependency on the number of 
input tokens in the request, $t_i^{\text{in}}$.

\paragraph{2. Pre-process.}
Upon arrival, the API server thread tokenizes the prompt, performs light
validation, and enqueues the request into the message queue contributing 
a latency of $u_i$. This has a linear dependence 
on the number of input tokens, $t_i^{\text{in}}$.

\paragraph{3. Scheduling.} The request waits in the queue until there is 
sufficient GPU capacity. The duration of this waiting period equals the
sum of the durations of the busy loop iterations, $\mathcal{S}_i$, within this period.

\paragraph{4. Chunked prefill.} The duration of prefill equals the
sum of the durations of the busy loop iterations, $\mathcal{P}_i$ in which the request 
participated in its prefill phase.

\paragraph{5. Decode.} The duration of decode equals the
sum of the durations of the busy loop iterations, $\mathcal{D}_i$, in which the request 
participated in its decode phase.

\paragraph{6. Post-process.} Once all output tokens are produced, 
the API server detokenizes them and
packages the final response. This incurs a post-process latency $c_i$, 
with a linear dependence on the number of output tokens $t_i^{\text{out}}$.
...
dependence on the number of output tokens $t_i^{\text{out}}$.
...
We first define the quadratic term that captures the quadratic size of the the 
KV tensors that are computed in the presence of cache misses and 
cache hits for requests during iteration $j$:
\begin{align}
\psi_j
...
We can now model the duration of iteration $j$ 
